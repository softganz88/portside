...
- **Releases:** the version lives in `package.json`, `src-tauri/Cargo.toml` and `src-tauri/tauri.conf.json`, so bump all three together; bundle filenames come from `tauri.conf.json`. Ship the AppImage only after `npm run repack-appimage`, because Tauri's own AppImage is ~0.5 s slower to start and Tauri can't pass the packing options itself. Right after `gh release create`, the by-tag view (`gh release view`/`download`) can list no assets for a while. Check `gh api repos/<owner>/portside/releases/<id>/assets` before re-uploading.
